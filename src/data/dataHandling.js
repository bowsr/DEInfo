...
function getValue(demon, key, versions, target, current = null) {
    ...
    if(d === undefined) return (current.prev === -1) ? null : getValue(demon, key, versions, target, getVersionFromID(versions, current.prev));
    var value;
    switch(key) {
        case 'health':
            value = d.hp;
    ...
    if(value === null) return (d.intro) ? null : getValue(demon, key, versions, target, getVersionFromID(versions, current.prev));
    if(!d.intro && current.id > target) return getValue(demon, key, versions, target, getVersionFromID(versions, current.prev))
    return value;
}
